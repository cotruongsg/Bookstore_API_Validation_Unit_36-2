...
describe("PATCH /books/:id", function () {
    test("Updates partial a single book", async function () {
        const response = await request(app)
            .patch(`/books/${book_isbn}`)
            .send({
            amazon_url: "https://www.amazon.com",
            author: "Truong",
            language: "english",           
            title: "UPDATED PARTIAL BOOK",
            year: 2023
            });
        expect(response.body.book).toHaveProperty("isbn");
        expect(response.body.book.title).toBe("UPDATED PARTIAL BOOK");
    })
});
